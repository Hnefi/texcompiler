...
public class KernelDriver extends Driver {
	static {
		...
		options.add("outtype", "--output file type: cl(OpenCL) or cu(CUDA)");
...
//			dr.run(args);
	...
	public void doOptimize(String[] args) throws UnsupportedCodeException {
		...
		String outtype = "cu";
		if (options.getValue("outtype")!=null) {
			outtype = options.getValue("outtype");
		}				
		
		if (!output.exists()) output.mkdir();
		String file = super.filenames[0];
		GProcedure proc =  new GProcedure(file, outtype);
