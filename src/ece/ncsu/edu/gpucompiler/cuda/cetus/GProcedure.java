...
import ece.ncsu.edu.gpucompiler.FileFormatter;
...
public class GProcedure {
	...
	static String[] PRE_DEFINE_NAME = {"COALESCED_NUM", "blockDimX", "blockDimY", "idx", "idy",
		"merger_y", "coalesced_idy", "globalDimX", "globalDimY"};
	
	static {
		PRE_DEFINE.put("COALESCED_NUM", "16");
		PRE_DEFINE.put("blockDimX", "1");
		PRE_DEFINE.put("blockDimY", "1");
		PRE_DEFINE.put("idx", "(bidx*blockDimX+tidx)");
		PRE_DEFINE.put("idy", "(bidy*blockDimY+tidy)");
...
//					defines.add(line);
	...
	public GProcedure(String file, String outtype) throws UnsupportedCodeException {
		try {
			this.outType = outtype;
...
//		preDefines;
		...
		if (rts.size()>=1) {
			...
			if (returntype!=Specifier.GLOBAL&&returntype!=Specifier.OPENCL_KERNEL) {
...
//			List sbs = func.getParameters();
			String type = FileFormatter.detect(func);
			System.out.println("detect " + type);
			if (type.equals(FileFormatter.FILE_UNKNOWN)) continue;
			if (type.equals(FileFormatter.FILE_OPENCL)) {
				func = FileFormatter.convertOCl2Cuda(func);
				System.out.println("convert to cuda");
			}
			result.add(func);
		}
		return result;
...
//			bw.write("struct float2 { float x; float y;};\n");
	...
	public String generateRunnableCode() {
		return generateRunnableCode(FileFormatter.FILE_CUDA);
	}
	

	
	public String generateRunnableCode(String type) {
		new Array2FunctionPass(true).dopass(this);
		Procedure outp = procedure;
		if (type.equals(FileFormatter.FILE_OPENCL)) {
			outp = FileFormatter.convertCuda2OCl(procedure);
		}
		String source  = outp.toString();
		new Array2FunctionPass(false).dopass(this);

		StringBuffer sb = new StringBuffer();
//		sb.append("struct float2 { float x; float y; };\n");
		
		// defines for different type file (opencl and cuda)
		Hashtable<String,String> typeDefs = FileFormatter.getDefTable(type);
		for (String def: FileFormatter.HEADER) {
			sb.append("#define "+def+" "+typeDefs.get(def)+"\n");
		}		
		// we need to first output PRE_DEFINE_NAME
		...
		for (String def: defines.keySet()) {
			...
			for (String def1: PRE_DEFINE_NAME) {
				if (def1.equals(def)) isdone = true;
			}
			for (String def1: FileFormatter.HEADER) {
				if (def1.equals(def)) isdone = true;
			}
	...
	public File gerenateRAWOutput(File folder, String filename, String type) {
		filename = filename.replaceAll("\\[|\\]|\\.|\\=|\\-|\\*|\\:|\\;|\\+|\\(|\\)", "_");
		try {
			File file = new File(folder, "gcompiler_"+filename+"."+outType);
			BufferedWriter bw = new BufferedWriter(new FileWriter(file));
			bw.write(generateRunnableCode(type));
			bw.close();
			return file;
		} catch (IOException e) {
			e.printStackTrace();
	...
	public File gerenateOutput(File folder, String filename) {
		...
		try {
			...
			lupass.dopass(this);
			bw.write(generateRunnableCode(outType));
			bw.close();
			return file;
		} catch (IOException e) {
			e.printStackTrace();
