...
runTest(int argc, char** argv)
{
...

    {
		...
		// execute the kernel
		matmul_naive<<< grid, threads >>>(d_A, d_B, d_C, WA, WB);
		...
		printf("matmul_naive Processing time: %f (ms), %f Gflops \n", cutGetTimerValue(timer), 2000.0*MW*MW*MW/cutGetTimerValue(timer)/1024/1024/1024);
...

    {
		...
		CUT_SAFE_CALL(cutStartTimer(timer));
		// execute the kernel
		cublasSgemm('t', 't', MW, MW, MW, 1.0f, d_A,
...

    {
		...
		CUT_SAFE_CALL(cutStartTimer(timer));
		// execute the kernel
		matmul_coalesced<<< grid, threads >>>(d_A, d_B, d_C, WA, WB);
		// stop and destroy timer
		cudaThreadSynchronize();
		CUT_SAFE_CALL(cutStopTimer(timer));
		...
		printf("matmul_coalesced Processing time: %f (ms), %f Gflops \n", cutGetTimerValue(timer), 2000.0*MW*MW*MW/cutGetTimerValue(timer)/1024/1024/1024);
		CUT_SAFE_CALL(cutDeleteTimer(timer));
	    CUDA_SAFE_CALL(cudaFree(d_C));
    }
    // check result
...

    {
		...
		CUT_SAFE_CALL(cutStartTimer(timer));
		// execute the kernel
		matmul_opt<<< grid, threads >>>(d_A, d_B, d_C, WA, WB);
		// stop and destroy timer
		cudaThreadSynchronize();
		CUT_SAFE_CALL(cutStopTimer(timer));
		...
		printf("matmul_opt Processing time: %f (ms), %f Gflops \n", cutGetTimerValue(timer), 2000.0*MW*MW*MW/cutGetTimerValue(timer)/1024/1024/1024);
		CUT_SAFE_CALL(cutDeleteTimer(timer));
	    CUDA_SAFE_CALL(cudaFree(d_C));
    }
    // check result
